...
from rnet.params import Params
...
parser.add_argument('-p','--process', default = True, type = str2bool, help='Use the coreNLP tokenizer.', required=False)
parser.add_argument('-r','--reduce_glove', default = True, type = str2bool, help='Reduce glove size.', required=False)
...
class data_loader(object):
    ...
    def process_json(self,file_dir,out_dir, write_ = True):
        ...
        with codecs.open("dictionary.txt","wb","utf-8") as f:
            for key, value in sorted(self.w_dict.items(), key = lambda x:x[1],reverse = True):
    ...
    with codecs.open(dir_, "rb", "utf-8") as f:
        ...
        while line:
            ...
            if len(vector) != Params.emb_size + 1:
                line = f.readline()
                continue
            vocab = normalize_text(''.join(vector[0:-Params.emb_size]))
            if vocab not in dict_:
                line = f.readline()
                continue
            glove_f.append(line)
