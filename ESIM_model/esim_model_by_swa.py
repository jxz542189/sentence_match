...
from utils.moving_free_batch_normalization import moving_free_batch_norm
...
def batch_norm(inputs, is_training, use_moving_statistics):
    if _MOVING_FREE_BATCH_NORM:
        return moving_free_batch_norm(inputs=inputs,
                                      axis=-1,
    ...
    def _feedForwardBlock(self, inputs, hidden_dims, num_units,
        ...
        with tf.variable_scope(scope, reuse = isReuse):
            ...
            with tf.variable_scope('feed_foward_layer2'):
                ...
                results = batch_norm(results, self.is_training, self.use_moving_statistics)
                return results
